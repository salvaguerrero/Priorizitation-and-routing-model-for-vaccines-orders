...
def ahp_comp(Deley,AMD,t):

  #AHP data:
  FAM_df = pd.read_csv('data\hungerindex_clean.csv')  
  GE_df = pd.read_csv('data\GE.csv')  
  NUR_df = pd.read_csv("data/nursing_10000_clean.csv")  
  HE_df = pd.read_csv('data\health_exp_clean.csv')  
  ...
  def AHP_weight(df):
    ...
    #aux['Total'] = aux.iloc[:aux.shape[1]-1,0].div(4, axis=0)
  ...
  for ci in co_code:
    a = NUR_df[FAM_df['ISO']==ci]
    ii = a['val'].values[0]
    for vi in vac:
        
          #column index: j
          for cj in co_code:
            a = NUR_df[FAM_df['ISO']==cj]
            jj = a['val'].values[0]
            for vj in vac:
                  A[i,j] = ii - jj
                  j = j + 1                   
  ...
  for ci in co_code:
    a = HE_df[HE_df['Co_code']==ci]
    ii = a['2020'].values[0]
    for vi in vac:
        
          #column index: j
          for cj in co_code:
            a = HE_df[HE_df['Co_code']==cj]
            jj = a['2020'].values[0]
            for vj in vac:
